...
3.  **元のデータシート**（CSVからインポートしたシート）に戻り、47都道府県の「**都道府県**」、「**出生数**」、「**婚姻件数**」の3列をコピーし、「2変数分析」シートのA1セルに貼り付けます。
...
[*近似曲線*とは？](https://qiita.com/Sakai_path/items/8040beeb6c857665b604)


---

## ステップ3：相関係数の計算プロセスを理解する【修正版】

散布図で見た関係性を、`=CORREL`関数を使わずに計算してみましょう。これにより、相関係数がどのように成り立っているかが分かります。
...
* `H2`〜`J2`セルを選択し、フィルハンドルで下までコピーします。
...
* `L2`セルに「**出生数 標準偏差**」と入力し、`M2`セルに以下を入力します。
    * `=SQRT(SUM(I2:I48)/(47-1))`
* `L3`セルに「**婚姻件数 標準偏差**」と入力し、`M3`セルに以下を入力します。
    * `=SQRT(SUM(J2:J48)/(47-1))`

### (5) 相関係数を計算する
相関係数は、「共分散」を「2つの標準偏差の積」で割ったものです。
* `L5`セルに「**相関係数（手計算）**」と入力し、`M5`セルに以下を入力します。
    * `=M1/(M2*M3)`

これで相関係数が計算できました。

相関係数を「計算」しましたが、その数値の「解釈」はもっと重要です：**[相関係数（r）が何を意味しているのか？](https://addone.github.io/datascience-gt/correlation_simulator)** （Excelでとても簡単なやり方もぜひチェックしてください）
...
**【最終確認】**
すべてのシートが正しく保存されていることを確認したら、ファイルを閉じ、Teamsの課題へ提出してください。
